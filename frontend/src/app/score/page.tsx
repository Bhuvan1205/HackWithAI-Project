...
export default function ScoreClaimPage() {
 ...
 const handleSubmit = async (e: React.FormEvent) => {
  ...
  try {
   ...
   const response = await fetch("http://127.0.0.1:8000/api/v1/score-intelligence", {
    method: "POST",
 ...
 return (
  <PageTransition className="p-6 md:p-8 max-w-4xl mx-auto w-full pt-12">
   ...
   <div className="mb-8 flex flex-col items-center text-center space-y-2">
    <div className="h-12 w-12 rounded-full border border-[var(--accent-color)]/30 bg-[var(--accent-color)]/10 flex items-center justify-center glow-accent mb-2">
     <FilePlus2 className="text-[var(--accent-color)]" size={24} />
    </div>
    <h1 className="text-3xl font-bold tracking-tight text-[var(--text-primary)] uppercase">Intelligence Profiling Engine</h1>
    <p className="text-[var(--text-secondary)]">Submit a new claim telemetry payload for real-time anomaly analysis.</p>
   </div>
    ...
    {status === '503' && (
     ...
     >
      ...
      <div className="text-left">
       ...
       <p className="text-xs mt-1 text-[var(--text-secondary)]">Intelligence pipeline is currently offline or saturated. Please stand by.</p>
      </div>
     </motion.div>
    )}
   </AnimatePresence>
      ...
      return (
       <div key={step.id} className="flex flex-col items-center gap-2 relative z-10 bg-background px-2">
        <div className={cn(
         ...
         isActive ? "bg-[var(--accent-color)] border-[var(--accent-color)] text-white shadow-[0_0_15px_var(--accent-color)] scale-110" :
          isCompleted ? "bg-[var(--bg-secondary)] border-[var(--accent-color)]/50 text-[var(--accent-color)]" :
        ...
        <span className={cn(
         ...
         isActive ? "text-[var(--text-primary)]" : isCompleted ? "text-[var(--text-primary)]/80" : "text-[var(--text-secondary)]"
        )}>
         {step.name}
        </span>
       </div>
   ...
   <Card className="glass-panel-heavy mt-12 overflow-hidden border-t-accent/50">
    <form onSubmit={handleSubmit}>
     ...
     <CardFooter className="flex justify-between border-t border-[var(--border-color)] bg-[var(--text-primary)]/5 p-4">
      <Button
       type="button"
       variant="ghost"
       onClick={handlePrev}
       ...
       <motion.button
        ...
        className={cn(
         ...
         status === 'idle' ? "bg-[#34C759] text-white hover:bg-[#34C759]/90 glow-green" :
          status === 'loading' ? "bg-[var(--text-primary)]/10 text-[var(--text-secondary)] cursor-not-allowed" :
           status === 'success' ? "bg-[#34C759] text-white shadow-[0_0_30px_rgba(52,199,89,0.8)]" :
            "bg-[var(--intel-danger)] text-white animate-shake glow-red"
        )}
      ...
      <Card className="mt-8 border-[#FF3B30]/50 glass-panel bg-[#FF3B30]/10 glow-red p-6">
       <div className="flex items-center gap-4">
        ...
        <div>
         ...
         <p className="text-sm text-[var(--text-secondary)]">The intelligence engine encountered a fatal runtime error.</p>
        </div>
       </div>
     ...
     >
      ...
      >
       <Card className="border-[#34C759]/50 glass-panel-heavy glow-green p-6 text-center space-y-4">
        ...
        <h3 className="text-xl font-black text-[var(--text-primary)] uppercase tracking-widest mt-4">Duplicate Claim Detected</h3>
        <p className="text-sm text-[var(--text-secondary)] pb-4">Telemetry signature for <span className="font-mono text-[var(--text-primary)]">{formData.claim_id}</span> has already been analyzed.</p>
