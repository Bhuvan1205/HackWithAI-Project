   ...
   <Card className={cn(
    ...
    <CardContent className="p-6 relative z-10 flex flex-col justify-between h-full">
     <div className="flex items-start justify-between">
      ...
      <div className={cn(
       "flex h-8 w-8 items-center justify-center rounded bg-[var(--text-primary)]/5 border border-[var(--border-color)]",
     ...
     <div className="mt-4 flex items-center text-xs">
      {trend !== undefined && (
       <span className={cn(
        ...
        trend > 0 ? "text-[var(--intel-danger)]" : trend < 0 ? "text-[#34C759]" : "text-[var(--text-secondary)]"
       )}>
        {trend > 0 ? <ArrowUpRight size={14} /> : trend < 0 ? <ArrowDownRight size={14} /> : <Minus size={14} />}
        {Math.abs(trend)}%
       </span>
      )}
      {trendLabel && <span className="text-[var(--text-secondary)] ml-2">{trendLabel}</span>}
     </div>
    </CardContent>
   </Card>
  </motion.div>
